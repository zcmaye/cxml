...
//节点栈
std::stack<CXMLNode *> st;

//单口节点
const std::string open_labels[] = {"br", "hr", "img", "input",
                              "meta", "area", "base", "col",
...
std::string find_rigth_element_name(const std::string cxml)
{
    std::string name;
    std::string str = cxml;
    size_t pos = str.find("</");
...
CXMLNode *parse_node_element_attr(const std::string cxml, CXMLNode *root)
{
    ...
    std::string _str = str.substr(str.find(root->name) - 1, str.find('>') + 1);
    size_t equal_pos = 0;
    while (_str.find('=') != equal_pos && _str.find('=') < maxLength)
    {
        ...
        strip(attr_value);
        attrs->attributes.insert(std::pair<std::string, std::string>(attr_name, attr_value));
        attrs->nums++;

        //std::cout << "属性名:" << attr_name << " "<< "属性值:" << attr_value << std::endl;
...
CXMLNode *parse_node(const std::string cxml, CXMLNode *root)
{
    ...
    else
    {
        ...
        //std::cout << "栈顶元素:" << st.top()->name << std::endl;
        //std::cout << st.top()->name << "子加入:" << root->name << std::endl;
...
// {
//     //std::cout << (root->name == target) << std::endl;
//     if (root->name == target)
//     {
//         return root;
//     }
